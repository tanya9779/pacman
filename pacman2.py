    ...
    def game_tick(self):
        ...
        if floor(self.x)!=floor(new_x) or floor(self.y)!=floor(new_y):
            ...
            if map.moveTo(self, new_x, new_y ): # если стены не мешают передвинуться
                point = map.get(new_x, new_y)# это список
                ball = None # мы перебираем список point сразу удалять объекты нельзя - запомним их
                bonus = None #поэтому изменять список и
                ghost = None # сразу удалять объекты нельзя - запомним их
                for obj in point: # в клетке могут оказаться разные объекты
                    if isinstance(obj,Ball):
                        ball = obj #чуть позже удалим с карты
...
if __name__ == '__main__':
    ...
    ghost2 = Ghost(0,0, tile_size,map_size) #картинка приведения только одна- перекрасим его в синий
    pxarr = pygame.PixelArray(ghost2.image) #подсмотренно в интернете
    pxarr.replace(RED, BLUE, 0.5) #как перекрашивать
    del pxarr #важно удалить объект PixeArray- иначе не работает дольше
    ...
    while 1: # здесь основной цикл
        ...
        if not pacman.is_alive():#pacmana сожрали
            # FIXME -ВЫВЕСТИ СОЖАЛЕНИЕ
            pygame.time.delay(3000)
            break
        if map.point_count==0 and map.bonus_count==0:#собрали все шары и все бонусы
            # FIXME -ВЫВЕСТИ ПОЗДРАВЛЕНИЕ Вы собрали всё
            pygame.time.delay(3000)
            break
