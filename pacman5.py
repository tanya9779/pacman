                ...
                if obj in point:
                        ...
                        for obj2 in n_point:
                            if isinstance(obj2, Wall): # это стена
                                if isinstance(obj, Ghost) or isinstance(obj, SmartGhost):
                                    return False # приведение через стены не ходит
                                elif isinstance(obj, Pacman): 
                                    if obj2.is_solid(): # через неразруш стену ходит супермен
                                       if obj.crash and obj.health >= obj.superMan:
                                           obj.health -= 500 # на проход через такую стену тратится энергия
                                           obj.checkPower()
                                       else:
                                           return False
                                    else: # Pacman может разрушить разрушаемую стену
...
# Умное привидение (синее)
# умеет подглядывать за пакманом
# когда пакман в той же строке или столбце - привидение движется к пакману (через стены только видит)
# ходить через стены не может
class SmartGhost(GameObject):
    ...
    def game_tick(self):
        ...
        if floor(self.x) == floor(pacman.x): # выбираем направление "к пакману"
            if pacman.y > self.y:
                self.direction =2
            else:
                self.direction = 4
             ...
             if pacman.x > self.x:
                 ...
                 self.direction = 3
        else: # пакман в другой строке/столбце
...
class Pacman(GameObject):
    def __init__(self, x, y, tile_size, map_size):
        ...
        self.direction = 0
        self.velocity = 4.0 / 10.0
        self.superMan = 5000 # после достижения такой энергии пакман может пользоваться гиперпространством
        self.crash = False # при нажатии SHIFT можно пройти сквозь стену если хватает здоровья
    ...
    def checkPower(self): # вызывается, если изменился уровени энергии - пакман может перекрашиваться
                          # и  увеличивать и уменьшать скорость

        if self.health >= self.superMan: # энергии достаточно для "Покраснения" пакмана
            pxarr = pygame.PixelArray(self.image) 
            pxarr.replace(YELLOW, RED, 0.5) 
            del pxarr # важно удалять объект PixelArray - иначе не работает дольше
            self.velocity = (0.0+self.health) / 10000.0 # увеличим скорость
        else: # энергия уменьшилась - опять желтый
            pxarr = pygame.PixelArray(self.image) 
            pxarr.replace(RED, YELLOW, 0.5) 
            del pxarr 
            self.velocity = 4.0 / 10.0
    ...
    def game_tick(self):
        ...
        if self.direction == 1:
            new_x = self.x + self.velocity
            if new_x >= self.map_size-1:
                if self.health >= self.superMan: # когда энергии полно - можно перепрыгнуть
                                                 # через гиперпространство (SHIFT прижать)
                    new_x = 0
                else:
                    new_x = self.map_size-1
        elif self.direction == 2:
        ...
        if floor(self.x)!=floor(new_x) or floor(self.y)!=floor(new_y): # вышли в другую клетку
            ...
            if map.moveTo(self, new_x, new_y ): # если стены не мешают передвинуться
                ...
                for obj in point: # в клетке могут оказаться разные объекты
                    ...
                    elif isinstance(obj,Ghost) or isinstance(obj,SmartGhost):
                        ghost = obj
